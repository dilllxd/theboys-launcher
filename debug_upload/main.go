...
func main() {
	...
	// Try to extract file ID using updated patterns
	html := string(body)
	fmt.Println("\n*** Testing extraction patterns ***")

	// Pattern 1: href="https://logs\.dylan\.lol/upload/([^"]+)"
	...
	// Pattern 2: href="https://logs\.dylan\.lol/file/([^"]+)"
	...
	// Pattern 3: href="https://logs\.dylan\.lol/edit/([^"]+)"
	editPattern := `href="https://logs\.dylan\.lol/edit/([^"]+)"`
	if matches := regexp.MustCompile(editPattern).FindStringSubmatch(html); len(matches) > 1 {
		fmt.Printf("Pattern 3 matched: %s\n", matches[1])
	} else {
		fmt.Println("Pattern 3: No match")
	}

	// Pattern 4: const url = .*https://logs\.dylan\.lol/upload/([^"]+)"
	jsPattern := `const url = .*https://logs\.dylan\.lol/upload/([^"]+)"`
	if matches := regexp.MustCompile(jsPattern).FindStringSubmatch(html); len(matches) > 1 {
		fmt.Printf("Pattern 4 matched: %s\n", matches[1])
	} else {
		fmt.Println("Pattern 4: No match")
	}
