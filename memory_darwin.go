...
func getSystemMemoryInfo() (*DarwinMemoryInfo, error) {
	debugf("Retrieving macOS system memory information")
	var totalMemory uint64
	var freeMemory uint64
	size := uint64(8)

	// Get total physical memory
	debugf("Calling sysctl for hw.memsize")
	...
	if err != 0 {
		debugf("Failed to get total memory via sysctl: %v", err)
		return nil, err
	}
	debugf("Total memory detected: %d MB", totalMemory/(1024*1024))

	// Get free memory (vm page free count)
	var freeCount uint64
	size = uint64(8)
	debugf("Calling sysctl for vm.page_free_count")
	_, _, err = syscall.Syscall6(
		syscall.SYS___SYSCTL,
...
func getAvailableMemoryMB() int {
	debugf("Getting available memory for macOS")
	...
	debugf("Available memory detected on macOS: %d MB", availableMB)
	return availableMB
}
